...
Version:0.0.3
Print Hard Copy Functionality Added
Now User Can Print Their Data
...
class ftion():
    ...
    def printtt(self):
        ...
        #filename = tempfile.mkdtemp("pfile.txt")
        print_file = open("pfile.txt","w+")
        if len(con)>0:
            imps=""
            for i in con:
                if i=="\\":
            ...
            for i in range(len(st)):
                stt = st[i].split("::::")
                print_file.write(stt[0])
                try:
                    if stt[1][0] == ' ':
                        stt[1] = stt[1][0].replace("")
                    print_file.write(stt[1])
                    print_file.write("\n")
                except IndexError:
                    pass
            print_file.close()
            try:
                os.startfile("pfile.txt","print")
            except:
                messagebox.showerror("Error", "Error while printing!!!")
        else:
